...
```
meta exec "any command" --exclude management_api_core,management_api_contracts
meta exec "any command" --include-only management_api_core,management_api_contracts
```
...
```
meta exec "git flow feature start my_feature" --include-only management_api_core,management_api_contracts
```

* Starts the feature "my_feature" in the modules
`management_api_core` and `management_api_contracts`

### 3.7 Publish a feature on multiple repositories

```
meta exec "git flow feature publish my_feature" --include-only management_api_core,management_api_contracts
```
...
```
management_api_meta/_integration_tests/node-modules/@process-engine/src/node_instance.ts
```
