    ...
    stage('Prepare') {
      steps {
        script {
          ...
          db_image       = docker.build("managementtest_db_image:${image_tag}", '--file _integration_tests/Dockerfile.database _integration_tests');
    ...
    stage('Management API Tests') {
      steps {
        script {
          ...
          server_image.inside("${node_env} ${management_api_mode} ${junit_report_path} ${config_path} ${db_process_model} ${db_timer} ${db_flow_node_instance} ${db_link}") {
